...
class WhatsAppClient {
  ...
  /**
   * Send an interactive list message (e.g. time slots or services).
   * sections: [{ title: 'Section title', rows: [{ id: 'value', title: 'Row title', description?: 'optional' }] }]
   * Max 10 rows total; buttonText is the CTA (e.g. "Pick a time").
   */
  async sendInteractiveList(phoneNumberId, accessToken, to, bodyText, buttonText, sections) {
    if (!phoneNumberId || !accessToken || !to || !sections?.length) return null;
    try {
      const formattedTo = to.replace(/[+\s]/g, '');
      const allRows = sections.flatMap(s => (s.rows || []).slice(0, 10));
      const rows = allRows.slice(0, 10).map(r => ({
      ...
      const action = {
        ...
        sections: [{ title: (sections[0].title || 'Options').slice(0, 24), rows }]
      };
      const interactive = {
        type: 'list',
        body: { text: String(bodyText).slice(0, 1024) },
      ...
      console.error('[WhatsApp] sendInteractiveList error:', error.response?.data || error.message);
      return null;
    }
  }
