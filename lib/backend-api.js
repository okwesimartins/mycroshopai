 ...
 * @returns {Promise<{ date, service_id, slots: Array<{ start, end, slot, label }> }|null>}
 */
async function getServiceAvailability(tenantId, serviceId, date, subscriptionPlan = 'enterprise') {
  if (!API_KEY) return null;
  try {
    const res = await axios.get(`${BASE_URL}/api/v1/ai-agent/service-availability`, {
      params: { tenant_id: tenantId, service_id: serviceId, date, subscription_plan: subscriptionPlan },
      headers: headers(),
      timeout: 10000
    });
