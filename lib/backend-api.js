...
/**
 * Attach a payment receipt image to a booking.
 * @param {number} tenantId
 * @param {number} bookingId
 * @param {string} receiptImageBase64  – base64-encoded image
 */
async function attachBookingReceipt(tenantId, bookingId, receiptImageBase64) {
  if (!API_KEY) throw new Error('AI_AGENT_API_KEY not set');
  try {
    const res = await axios.post(
      `${BASE_URL}/api/v1/ai-agent/attach-booking-receipt`,
      { tenant_id: tenantId, booking_id: bookingId, receipt_image_base64: receiptImageBase64 },
      { headers: headers(), timeout: 20000 }
    );
    return res.data?.success ? res.data : null;
  } catch (err) {
    console.error('[BackendAPI] attachBookingReceipt error:', err.response?.data || err.message);
    return null;
  }
}

/**
 * Approve or decline a booking payment (called after store owner taps button).
 * @param {number} tenantId
 * @param {number} bookingId
 * @param {'approve'|'decline'} action
 */
async function confirmBookingPayment(tenantId, bookingId, action) {
  if (!API_KEY) throw new Error('AI_AGENT_API_KEY not set');
  try {
    const res = await axios.post(
      `${BASE_URL}/api/v1/ai-agent/confirm-booking-payment`,
      { tenant_id: tenantId, booking_id: bookingId, action },
      { headers: headers(), timeout: 15000 }
    );
    if (res.data?.success) return { success: true, booking: res.data.data?.booking };
    throw new Error(res.data?.message || 'Confirmation failed');
  } catch (err) {
    const msg = err.response?.data?.message || err.message;
    console.error('[BackendAPI] confirmBookingPayment error:', msg);
    throw new Error(`Failed to confirm booking payment: ${msg}`);
  }
}

/**
 * Initialize a Paystack payment link for an order or booking.
 * @param {Object} payload - Full payload per API spec (tenant_id, amount, email, name, metadata, etc.)
 * @returns {Promise<{ success, payment_link, reference }|null>}
 */
async function initializePaymentLink(payload) {
  if (!API_KEY) throw new Error('AI_AGENT_API_KEY not set');
  try {
    const res = await axios.post(
      `${BASE_URL}/api/v1/ai-agent/initialize-payment-link`,
      payload,
      { headers: headers(), timeout: 20000 }
    );
    if (res.data?.success && res.data?.data) {
      return {
        success: true,
        payment_link: res.data.data.payment_link || res.data.data.authorization_url,
        reference:    res.data.data.reference,
      };
    }
    return null;
  } catch (err) {
    console.error('[BackendAPI] initializePaymentLink error:', err.response?.data || err.message);
    return null;
  }
}

/**
 * Submit a refund request on behalf of a customer.
 * @param {Object} data - { tenant_id, customer_name, customer_phone, customer_email,
 *                          order_id, receipt_id, source_channel, reason, details }
 * @returns {Promise<{ success, message }|null>}
 */
async function submitRefundRequest(data) {
  if (!API_KEY) throw new Error('AI_AGENT_API_KEY not set');
  try {
    const res = await axios.post(
      `${BASE_URL}/api/v1/ai-agent/refund-request`,
      data,
      { headers: headers(), timeout: 15000 }
    );
    return res.data?.success
      ? { success: true, message: res.data.message || 'Refund request submitted.' }
      : { success: false, message: res.data?.message || 'Refund request failed.' };
  } catch (err) {
    console.error('[BackendAPI] submitRefundRequest error:', err.response?.data || err.message);
    return { success: false, message: 'Could not submit refund request right now.' };
  }
}

/**
 * Get ALL unconfirmed (awaiting_payment / pending_approval) orders for a customer phone.
 * Used when customer sends a receipt and has multiple orders pending — we show them a
 * list so they can pick which order the receipt belongs to.
 *
 * Returns an array of order objects (may be empty). Each order should have:
 *   { id, order_number, total, items, created_at }
 *
 * Falls back to the single-order endpoint if the multi-order endpoint doesn't exist yet.
 */
async function getAllPendingOrdersByPhone(tenantId, customerPhone) {
  if (!API_KEY) return [];
  try {
    // Try the dedicated multi-pending endpoint first
    const res = await axios.get(`${BASE_URL}/api/v1/ai-agent/orders/all-pending-by-phone`, {
      params: { tenant_id: tenantId, customer_phone: customerPhone },
      headers: headers(),
      timeout: 10000,
    });
    ...
    // Endpoint may return a single order wrapped in data.orders as array
    if (res.data?.success && res.data?.data?.order) {
      return [res.data.data.order];
    }
    return [];
  } catch (err) {
    // If 404 (endpoint not yet deployed), fall back to the single-order endpoint
    if (err.response?.status === 404) {
      console.warn('[BackendAPI] getAllPendingOrdersByPhone: multi-pending endpoint not found, falling back to single');
      const single = await getPendingOrderByPhone(tenantId, customerPhone);
      return single ? [single] : [];
    }
    console.error('[BackendAPI] getAllPendingOrdersByPhone error:', err.response?.data || err.message);
    return [];
  }
}

/**
 * Check if a tenant is allowed to send a follow-up (and consume one credit).
 * Must be called immediately before sending each follow-up message.
 *
 * Returns the full API response so callers can log the reason on denial.
 * On network error, returns { allowed: true, _error: true } — fail open so
 * a temporary backend outage doesn't silence all follow-ups.
 */
async function checkFollowUpLimit(tenantId) {
  if (!API_KEY) return { success: false, allowed: false, reason: 'no_api_key' };
  try {
    const res = await axios.post(
      `${BASE_URL}/api/v1/whatsapp-plans/agent/use-followup`,
      { tenant_id: tenantId },
      { headers: headers(), timeout: 10000 }
    );
    return res.data;
    ...
    const status = err.response?.status;
    const body   = err.response?.data;
    console.error(`[BackendAPI] checkFollowUpLimit error: HTTP ${status || 'no-response'}`, body || err.message);
    // Fail open on network errors — don't silently drop legitimate follow-ups
    return { allowed: true, _networkError: true };
  }
}
